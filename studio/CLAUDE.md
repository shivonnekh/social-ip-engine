...
- **⚠️ The 即梦 hang-lottery (root-caused 2026-07-10, 236-task analysis).**
  ...
  **(5) added 2026-07-13 — a "late completer" is NOT rare, and it can be
  BETTER than the attempt that "won":** attempt 1 timed out at 10 min and got
  abandoned in favor of attempt 3, which succeeded fast — but attempt 1 had
  ACTUALLY finished ~15 min after we gave up on it, and it was the cleaner
  take (attempt 3 had 即梦's own auto-caption overlay burned in; this overlay
  is apparently probabilistic PER GENERATION, independent of the prompt —
  "NO subtitles" in the prompt does not reliably suppress it). The generation
  loop now re-checks every abandoned attempt (oldest first) after the retry
  loop ends and PREFERS the earliest one that actually completed, over
  whatever the loop landed on — zero extra 即梦 credits, and usually matches
  what's already sitting as a finished task in 即梦's own web dashboard by the
  time a human looks. **Corollary: 即梦's auto-caption burn-in is a real,
  unsolved intermittent problem** — always eyeball a regenerated shot (or at
  minimum spot-check a frame) before trusting it; there is no CLI flag or
  reliable prompt wording found so far that suppresses it outright.
...
- **New-account gotcha (hit live 2026-07-10):** a 即梦 account that has never
  used the digital-human / 全能参考 feature gets `ret=4010
  AigcComplianceConfirmationRequired` (or silent hangs) on CLI
  multimodal2video until a ONE-TIME compliance confirmation is done on the
  即梦 WEB UI (start any digital-human generation there once, accept the
  consent dialog). Two accounts exist — switch with `dreamina-switch
  旧号/新号` (`~/.local/bin/dreamina-switch`, token profiles swapped through
  the macOS keychain; auto-refreshes the stored token on every switch).
- **🚫 MEDICAL VOCABULARY IN PROMPTS = the main hang trigger (root-caused
  2026-07-10 by diffing prompts Shivonne verified PASS on the web UI against
  ours that hung).** doctor / patient / clinic / treatment / diagnosis /
  needles trip 即梦's medical content review → eternal "querying". Rules now
  baked into `notion_prompts.build_jimeng_prompt` (rewritten to the structured
  【Shot Guide】【Character】【Speech】【Style】【Camera】【Important】format
  that passed review): `sanitize_for_jimeng()` auto-swaps to wellness
  vocabulary (doctor→wellness educator, patient→participant, clinic→wellness
  studio, treatment→demonstration, acupuncture→acupressure…), plus explicit
  negative guards (No medical procedures / NO glow / NO halo / NO particles).
  ALL 168 existing Production-row 即梦 prompts were migrated in place on
  2026-07-10 via `scripts/update_jimeng_prompts.py` (non-destructive: patches
  ONLY the prompt code blocks, idempotent, handles all historical prompt
  formats — rerun it any time a row looks stale). When AUTHORING shot guides,
  prefer wellness wording from the start; for two-person shots add "the
  participant faces mostly away from the camera" (keeps the second face
  non-dominant — avoids the two-person lip-sync hang).
