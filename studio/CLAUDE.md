...
- `notion_video.py --row <id>` — 即梦 video: per shot, pull image+audio+即梦prompt from Notion → `dreamina multimodal2video` → download → place in "🎬 Video here" → merge final (merge is already automatic here — no separate manual "merge" step needed). `concat()` re-encodes via ffmpeg's concat FILTER at an explicit fixed fps (`_MERGE_FPS = 30`), NOT the concat demuxer's `-c copy` stream copy it used before 2026-07-08 — 即梦/Seedance shot outputs tag their container as 60fps nominal while actually playing back at ~24fps (confirmed via `avg_frame_rate` vs `r_frame_rate` on real shot files), and stream-copying shots whose own metadata already disagrees with itself risked a slow-motion artifact in the merged output (reported and fixed 2026-07-08). `concat()` also auto-runs `strip_ai_watermark()` on the merged `final.mp4` — 即梦 burns a mandatory "此视频由AI数字人技术生成，仅供参考" compliance label into a fixed-position strip near the bottom of every shot; this crops it off and zooms back to the original frame size, no manual step needed.
- `add_karaoke_captions.py --row <id> [--upload] [--script <path>]` — burns word-level karaoke-highlight captions (white base, current word yellow, black stroke, NO background box) onto that row's merged `final.mp4` → `final_karaoke.mp4`. Added 2026-07-07 to replace the previous ad-hoc process (JianYing CLI drafts kept failing to open; JianYing itself encrypts drafts since v6.0+ so a CLI-based draft the real app can open isn't currently viable on this machine — don't retry that path, see `scripts/memory/shello.md` 2026-07-07/08 sessions). **Uses moviepy, NOT ffmpeg's `ass`/`subtitles`/`drawtext` filters** — this machine's ffmpeg build has no libass/freetype support (`ffmpeg -filters` shows neither). Word timing comes from local `openai-whisper` (`word_timestamps=True`) run directly against the merged video's audio. Pass `--script <path-to-txt>` with the KNOWN correct VO script to fix Whisper mishearings (e.g. it transcribed "cramps" as "crampus", "Sanyinjiao" as "sanyan", "Stagnation" as "discation" in the period-pain campaign) while keeping Whisper's timestamps — see `align_to_known_script()`. Note this only corrects MISHEARINGS (same word count, different spelling); if the actual recorded VO genuinely omits/paraphrases part of the script (real gap found 2026-07-08 on this same campaign — several phrases in the "known" script were simply never spoken in the shots' audio), no amount of script-alignment fixes that — the shot's audio itself would need re-recording. `--upload` pushes the result to the row's **"Production Video" page PROPERTY** (not a body block) — the exact property social-ip-engine's live Reels auto-publish reads (`src/notion_publish.py::_extract_video_url`), so a captioned row is then one Stage-flip away from going live. Caches the Whisper transcript as `words.json` next to the video (gitignored, `campaigns/**/video/`) — pass `--retranscribe` to force a redo.
  - **Caption chunking/wrapping — several real bugs found via human review of live output on 2026-07-08, all fixed, worth knowing if captions ever look wrong again**: `group_words()` breaks a new caption chunk on `MAX_WORDS_PER_CHUNK=5` words, a >0.5s pause, OR a word ending in sentence-terminal punctuation (`.`/`!`/`?`) — that last rule was missing originally, so two unrelated sentences could land in one chunk if the speaker didn't pause between them. `wrap_chunk_to_lines()` picks the 2-line split with the most similar WIDTH between the two lines (not just the first split that fits) — a valid-but-lopsided split (e.g. "Treating them the" / "same is") reads as visually disconnected even though neither line overflows. Word glyphs need `margin=(0, word_vertical_pad)` padding (`WORD_VERTICAL_PAD_RATIO = 0.7`, ~34px at this font size) or MoviePy's TextClip clips off deep descenders (confirmed empirically: 'y'/'p'/'g' need ~32px+ headroom in Arial Black at font_size≈49, NOT the ~16px that looked sufficient when only tested against shallower letters) — and that padding must be measured OUT of `line_height` again before it's used for line-to-line spacing math, or the invisible padding itself inflates the visible gap between lines 3x.
