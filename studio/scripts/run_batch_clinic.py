#!/usr/bin/env python3
"""Drive the "Jackie treating a patient" batch through the pipeline, stage by stage.

Same driver as run_batch_2026_09.py, pointed at concepts_clinic_data. The one
behavioural difference is in stage_assets: these shots contain a RECURRING
GUEST as well as recurring props, so shots 2-3 are chained to shot 1 with
[SAME_PERSON_AS: Shot 1] in addition to [SAME_PROP_AS: Shot 1]. Shot 4 is
deliberately Jackie alone and is chained for PROPS ONLY — person-chaining it
would drag the guest into a frame she is not supposed to be in.
...
def stage_assets(keys: list[str], workers: int = 1) -> None:
    ...
    def one(key: str) -> tuple[str, bool, str]:
        ...
        # Shots 2-3 share BOTH the guest and the props with shot 1.
        lbl, ok, tail = run(["python3", "scripts/add_prop_markers.py", "--row", rid,
                             "--shots", "2,3", "--prop-ref", "1", "--person-ref", "1"], key)
        if not ok:
            return key, False, f"person+prop markers: {tail}"
        # Shot 4 is Jackie ALONE — props only. Chaining the person here would
        # reintroduce the guest into a frame that is meant to be a solo CTA.
        lbl, ok, tail = run(["python3", "scripts/add_prop_markers.py", "--row", rid,
                             "--shots", "4", "--prop-ref", "1"], key)
        if not ok:
            return key, False, f"prop markers (shot 4): {tail}"
