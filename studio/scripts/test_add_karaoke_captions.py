...
# ------------------------------------------ align_to_known_script(strict=True)


def test_align_strict_rebuilds_a_word_count_mismatch_from_the_script():
    """The real defect this mode fixes: Whisper misheard 'Daily Wei Qi tea:
    Astragalus' as a garbled, word-count-mismatched span. Non-strict mode
    would leave the wrong words on screen; strict mode must show the exact
    script text instead, since TTS audio has no ad-libbing to guess at."""
    words = [
        _word("A", 0.0, 0.2), _word("struggleous", 0.2, 0.9), _word("and", 0.9, 1.0),
        _word("Prince", 1.0, 1.3), _word("Ginseng", 1.3, 1.7),
    ]
    corrected = align_to_known_script(
        words, "Daily Wei Qi tea: Astragalus and Prince Ginseng", strict=True,
    )
    assert [w["word"] for w in corrected] == [
        "Daily", "Wei", "Qi", "tea:", "Astragalus", "and", "Prince", "Ginseng",
    ]
    # Matched tail ("and Prince Ginseng") keeps Whisper's own accurate timestamps.
    assert corrected[-1]["start"] == 1.3
    assert corrected[-1]["end"] == 1.7
    # Interpolated words stay within the mismatched span's time window and
    # are monotonically increasing (no overlaps, no going backwards).
    interpolated = corrected[:5]
    times = [w["start"] for w in interpolated] + [interpolated[-1]["end"]]
    assert times == sorted(times)
    assert interpolated[0]["start"] >= 0.0
    assert interpolated[-1]["end"] <= 1.0


def test_align_strict_still_corrects_simple_same_length_mishearing():
    words = [
        _word("If", 0.0, 0.28), _word("you", 0.28, 0.4), _word("get", 0.4, 0.58),
        _word("period", 0.58, 0.94), _word("crampus", 0.94, 1.48),
    ]
    corrected = align_to_known_script(words, "If you get period cramps", strict=True)
    assert [w["word"] for w in corrected] == ["If", "you", "get", "period", "cramps"]
    assert corrected[4]["start"] == 0.94
    assert corrected[4]["end"] == 1.48


def test_align_strict_drops_a_heard_word_the_script_never_said():
    """A 'delete' span (Whisper heard something with no script counterpart,
    e.g. a stray breath sound) has nothing to interpolate — script is
    ground truth, so it's correctly dropped, not invented."""
    words = [_word("If", 0.0, 0.2), _word("um", 0.2, 0.3), _word("you", 0.3, 0.5)]
    corrected = align_to_known_script(words, "If you", strict=True)
    assert [w["word"] for w in corrected] == ["If", "you"]


def test_align_strict_result_always_matches_script_word_count():
    words = [_word(f"w{i}", i, i + 0.5) for i in range(5)]
    corrected = align_to_known_script(
        words, "totally different script text here now", strict=True,
    )
    assert len(corrected) == 6  # matches the SCRIPT's word count, not Whisper's
