...
## Session — 2026-07-10 (correction + final wrap)
CORRECTION to the earlier 2026-07-10 entry: tcm-jessica-db-2 is NOT free tier — it was created 2026-06-26 directly on paid basic_256mb (~$6/mo, 15GB disk, expires: None). The "expires ~2026-09-24, must upgrade" warning was wrong (assumed free without checking the plan field). NO db expiry action needed, ever.
Also closed this session: (1) RECONCILE re-enable automated via RECONCILE_NOT_BEFORE=2026-07-13 server-side date gate (self-expiring, checked per sweep iteration, unparseable fails open+loud) — RECONCILE_ENABLED=true already set, nothing for a human to remember. (2) 🚀 Posted to FB checkbox column created on Production Tracker via API — retry queue will auto-tick. (3) FB Reel covers: video_reels API has no cover param (FB auto-picks a frame); fixed via POST /{video_id}/thumbnails is_preferred=true post-publish (verified live on video 1343882013854078, then wired into fb_publish.set_video_thumbnail + runner). (4) Period Pain republished BOTH platforms with correct cover + 🏷️ Title caption: IG reel DamdJwJARCN (old reel DakgOKwCh8P kept, user's choice), FB media 1343882013854078. Delete-and-republish playbook: delete FB via API (IG not deletable via API), clear BOTH ledger entries, DEPLOY (server disk ledger ≠ git origin — a trigger hitting the old instance right after "live" flips silently no-ops; retry the trigger), then POST /admin/notion-publish twice (IG first, FB mirror needs a second trigger after IG's ledger shows published).
Open items: NONE. All known time bombs defused.
