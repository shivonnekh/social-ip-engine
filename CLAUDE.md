...
**Notion → keyword auto-sync:** `POST /admin/notion-sync` (shared secret `X-Sync-Secret` header) reads Production Tracker directly (stdlib only, no import from `ai-tcm-ip` — separate repo/deploy), drafts a keyword rule the moment a row's `Stage` flips to `🟢 Ready to Publish` (pre-flight — armed before the post goes out; `✅ Published` still wireable as a safety net), writes straight to `comment_responses.json` (takes effect immediately, no redeploy — `comment_rules.load_rules()` reloads on file mtime) and best-effort pushes to git for durability (`GITHUB_PUSH_TOKEN`). `src/notion_sync.py`. Idempotent via `data/channels/notion_sync_state.json`.

**Notion checkbox write-back:** once a row's rule is durably saved to `comment_responses.json` (and only then — see ordering note below), `notion_sync._mark_wired()` PATCHes that same row's Notion checkbox property (default column name `🔗 DM Wired`, override via `NOTION_WIRED_CHECKBOX_PROP`) to `True` — so you see it ticked live on the Production Tracker itself instead of checking `git log` for the `chore: notion-sync — N new keyword rule(s)` auto-commit. **The column must already exist on the database as a Checkbox property** (create it once in Notion — this code never alters the database schema); it also needs "Update content" capability on the integration token, not just "Read content" — check this at notion.so/my-integrations. Until it's set up, every sync emits a harmless `mark_wired_failed` warning and the rule still ships normally (a raw transport error, not just an HTTP error, is caught too — a checkbox hiccup on one row can never cost a *different* row in the same batch its already-drafted rule). Disable with `NOTION_SYNC_MARK_WIRED=0`. A row whose checkbox PATCH fails is recorded in `data/channels/notion_wired_pending.json` and retried automatically at the top of every subsequent sync (never re-drafting the rule — only the checkbox is retried) until it succeeds. Rows wired before this feature existed are NOT retro-ticked (they're already in `notion_sync_state.json`, so never revisited).

**Ordering guarantee (do not break):** checkbox PATCHes for newly-wired rows are collected during the row loop but deliberately fired **after** `comment_responses.json` + `notion_sync_state.json` are both saved to disk — never inside the loop, never before the save. If a checkbox were ticked before the save and a LATER row in the same batch threw an uncaught exception, the process would crash before reaching the save, leaving an earlier row's checkbox permanently (and wrongly) showing "wired" in Notion while its DM rule was never actually persisted — the exact failure this module exists to prevent, just moved into the checkbox itself. `_save_json` also writes atomically (tmp file + `os.replace`) so a crash mid-write can't corrupt `comment_responses.json` either. See `test_sync_once_never_ticks_checkbox_before_rules_are_persisted` in `tests/test_notion_sync.py` — this is the regression test that pins this contract down; do not change the order of operations in `sync_once()` without keeping that test green.
...
- **Aspect ratio gotcha (learned the hard way, 2026-07-07):** Instagram's Content Publishing API only accepts photos between 4:5 and 1.91:1 aspect ratio. The DM-infographic convention (`notion_infographic_gen.py`, 1024x1536 = 0.667 ratio) is OUTSIDE this range and gets **rejected** for a feed/carousel post — generate at 1024x1024 (square) for anything destined for the feed, not the usual DM-infographic portrait.
- **Images must be publicly fetchable BEFORE any container call** — Meta fetches `image_url`/`video_url` server-side. A locally-generated file has to land in `data/media/`, then get **committed, pushed, and deployed** (manually trigger via the Render API — the auto-deploy webhook is broken, see §6) before the Graph API can see it. There's no shortcut here; a container call against a URL that 404s just fails.
- **The `--confirm-publish` safety-gate pattern** (see `publish_pressure_points_carousel.py`): prep everything (containers, poll to `FINISHED`, print the exact caption + which images) and stop — require a separate explicit flag/step for the actual `media_publish` call. Copy this pattern for any future one-off live-publish script; the point-of-no-return should never be reachable by the same command that does prep work.
- **Comment-keyword CTAs need their OWN rule.** If a caption says "comment X and I'll DM you," that only works if `X` already has an entry in `comment_responses.json` — a caption CTA and a comment rule are two separate things that must be added together, or the CTA is silently dead (per the "no rule match = silent" behavior above).
