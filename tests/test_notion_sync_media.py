...
def test_find_infographic_honours_studio_pipeline_toggle() -> None:
    """A row built by studio/'s apply_shot_plan holds its infographic in a
    toggle literally named "🖼️ Infographic here" (the "📊 DM Infographic"
    text sits on a sibling heading_3, which has no children).

    Before this was handled, ``find_infographic_source`` matched only the
    substring "dm infographic" and therefore returned None for every such
    row — so ``enrich_rule`` fell through to _generate_infographic() and
    produced a SECOND, different image from the Brief. Net effect: a real
    image-API call was spent per row, and the picture actually DM'd to
    viewers was NOT the one the human had reviewed in Notion, silently
    defeating the whole review checkpoint. Observed live on the
    "Dark Circles & Eye Bags" row, 2026-09-01.
    """
    tree = {
        ROW_ID: [
            {"id": "h1", "type": "heading_3", "heading_3": _rt("📊 DM Infographic")},
            {
                "id": "p1",
                "type": "paragraph",
                "paragraph": _rt("🖼️ Infographic prompt (→ GPT image gen)"),
            },
            _toggle_block(text="🖼️ Infographic here"),
        ],
        "h1": [],
        TOGGLE_ID: [_external_image_block("https://cdn.example/reviewed.png")],
    }
    assert (
        nsm.find_infographic_source(ROW_ID, _children_fn(tree))
        == "https://cdn.example/reviewed.png"
    )


def test_find_infographic_prefers_reviewed_toggle_over_written_back_one() -> None:
    """When BOTH toggles are present — the studio-pipeline one a human
    reviewed, and a "📊 DM Infographic here" toggle written back by a
    previous server-side generation — the human-reviewed image wins,
    regardless of document order. The write-back copy is a derivative; the
    reviewed one is the source of truth.
    """
    tree = {
        ROW_ID: [
            _toggle_block(block_id="tg-written", text="📊 DM Infographic here"),
            _toggle_block(block_id="tg-reviewed", text="🖼️ Infographic here"),
        ],
        "tg-written": [_external_image_block("https://cdn.example/server-generated.png")],
        "tg-reviewed": [_external_image_block("https://cdn.example/reviewed.png")],
    }
    assert (
        nsm.find_infographic_source(ROW_ID, _children_fn(tree))
        == "https://cdn.example/reviewed.png"
...
def test_find_infographic_prefers_reviewed_toggle_in_either_document_order() -> None:
    """Same guarantee as the test above, with the two toggles swapped.

    Together the pair pins TRUST order specifically. Either case alone can be
    satisfied by an implementation that merely iterates in a fixed direction
    (e.g. reverse document order with no exclusion rule) and happens to hit
    the reviewed toggle first; only running both orderings rules that out.
    """
    tree = {
        ROW_ID: [
            _toggle_block(block_id="tg-reviewed", text="🖼️ Infographic here"),
...
def test_find_infographic_falls_back_to_write_back_toggle_when_alone() -> None:
    """A row synced BEFORE the studio slot existed has only the write-back
    toggle. That legacy path must keep resolving exactly as it always did —
    the trust order adds a preferred slot, it must not drop the fallback.
    """
    tree = {
        ROW_ID: [_toggle_block(block_id="tg-written", text="📊 DM Infographic here")],
        "tg-written": [_external_image_block("https://cdn.example/server-generated.png")],
    }
    assert (
        nsm.find_infographic_source(ROW_ID, _children_fn(tree))
        == "https://cdn.example/server-generated.png"
    )
